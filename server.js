...
io.sockets.on('connection', function(socket) {
	...
	*/ 
	...
	/* This is the send_message command */
	/* payload:
		{
			'room': room to join,
		...
		{
			'result': 'success',
			'username': username of the person sending message,
			'message': the message to send
		}
		or
		{
	...
	*/
	socket.on('send_message',function(payload){
		log('server receieved a command', 'send_message', payload);
		if(('undefined' === typeof payload) || !payload){
...
/******* do I add uninvite to this code section? **********/
		var room = players[socket.id].room;
		var roomObject = io.sockets.adapter.rooms[room];
		/* Make sure the user being invited is in the room */
		if(!roomObject.sockets.hasOwnProperty(requested_user)) {
...
function send_game_update(socket, game_id, message) {
	...
	do {
		...
		if(numClients > 2) {
			...
			/* Kick on of the extra people out */
	...
	if((games[game_id].player_white.socket != socket.id) && (games[game_id].player_black.socket != socket.id)) {
		...
		/* and there isn't a color to given them */
