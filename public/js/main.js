...
socket.on('send_message_response',function(payload){
	...
	var newHTML = '<p><strong>' +payload.username+ ' says: </strong>' +payload.message+ '</p>';
	var newNode = $(newHTML);
	newNode.hide();
	$('#messages').prepend(newNode);
	newNode.slideDown(1000);
		...
		for(column = 0; column < 8; column++){
			...
			if(old_board[row][column] != board[row][column]) {
				if(old_board[row][column] == '?' && board[row][column] == '') {
					...
					$('#'+row+'_'+column).html('<img src="assets/images/empty_to_wht.gif" alt="white token" />');
				}
				else if(old_board[row][column] == '?' && board[row][column] == 'b') {
					$('#'+row+'_'+column).html('<img src="assets/images/empty_to_blk.gif" alt="black token" />');
				}
				else if(old_board[row][column] == '' && board[row][column] == 'w') {
					$('#'+row+'_'+column).html('<img src="assets/images/empty_to_wht.gif" alt="white token" />');
				}
					...
					$('#'+row+'_'+column).html('<img src="assets/images/wht_to_empty.gif" alt="empty square" />');
				}
